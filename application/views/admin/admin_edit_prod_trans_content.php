			...
			<div class="col-lg-12" id="content">
				<div id="breadcrumb-div">
					<ul class="breadcrumb">
					    ...
					    <li><a href="<?=base_url('admin/view-prod-trans/' . $prod_trans_id.'/'.$bc_id.'/'.$process_type_id.'/'.$year)?>">Sales BOM Transaction Details </a></li>
					    <li class="active">Edit Sales BOM Transaction (<?=$bc->bc_name?>)</li>
					    <?php } else { ?>
					    <li><a href="<?=base_url('admin/view-prod-trans/' . $prod_trans_id.'/'.$bc_id.'/'.$process_type_id.'/'.$year)?>">Production Transaction Details </a></li>
					    <li class="active">Edit Production Transaction (<?=$bc->bc_name?>)</li>
					    <?php } ?>
					</ul>
				</div>
				...
				<form method="post" action="<?=base_url('admin/update-prod-trans')?>" id="">
								...
								foreach($prod_trans as $row){
									...
									for ($i=1; $i <=12 ; $i++) {
										...
										if($row->process_type_id == 2){ //CLASSIFICATION
											...
											$prod_trans_output .= "<td class='text-center'><input type='text' name='cost[".$i."][]' size='6' class='form-control input-sm text-right' value='".$ave_wgt."'/></td>"; //MUST BE DYNAMIC BASED ON PROCESS TYPE
										} else {
