			...
			<div class="col-lg-12" id="content">
				...
				<form method="post" action="<?=base_url('production/update-ext-prod-trans')?>" id="">
						...
						<table class="table table-hover table-bordered table-stripe nowrap" id="tbl-new-ext-prod">
							...
							<tbody>
								...
								foreach($ext_prod_trans as $row){
									...
									$ext_prod_trans_output .= '<td>' .$row->material_desc .'</td>';
									for ($i=1; $i <=12 ; $i++) {
										
										$month = date('M', strtotime($year.'-'.$i.'-01'));
										$ext_prod_trans_output .= "<td class='text-center'><input type='text' name='ave_wgt[".$i."][]' size='6' class='form-control input-sm' value='".number_format(get_data('ext_prod_trans_dtl_tbl a', array('a.ext_prod_trans_id' => $row->ext_prod_trans_id, 'MONTH(a.trans_dtl_date)'	=>	$i), true, 'a.ave_wgt')->ave_wgt,3,'.',',')."'/>";
										
										$ext_prod_trans_output .= "<td class='text-center'><input type='text' name='cost[".$i."][]' size='6' class='form-control input-sm' value='".number_format(get_data('ext_prod_trans_dtl_tbl a', array('a.ext_prod_trans_id' => $row->ext_prod_trans_id, 'MONTH(a.trans_dtl_date)'	=>	$i), true, 'a.cost')->cost,2,'.',',')."'/></td>";
									}
