  				...
  				<div class="tab-content">
					...
					<div id="broiler-trans-tab" class="tab-pane fade in active">
						...
						<div id="add-btn">
							...
							<a href="<?=base_url('business-center/new-broiler-trans/' . encode($bc_id).'/'.$year)?>" class="btn btn-success btn-xs" id="add_broiler_trans_button">+ ADD BROILER TRANSACTION </a>
							
							<a href="<?=base_url('business-center/view-broiler-summary/' . encode($bc_id) .'/'. $trans_year)?>" id="view_broiler_summary" class="btn btn-info btn-xs">VIEW BROILER COST SUMMARY</a>
							...
							<table class="table table-hover table-bordered table-stripe nowrap" id="tbl-broiler-trans">
								...
								<tbody>
									...
									<tr>
										...
										<td class="text-center"><a href="<?=base_url('business-center/view-broiler-trans/' . encode($row->bc_id).'/'. encode($row->broiler_group_id).'/'.encode($row->broiler_group_name) . '/' . encode(date( 'Y', strtotime($row->broiler_trans_date))))?>" class="brn btn-xs glyphicon glyphicon-file edit-broiler-config" title="View"></a>&nbsp;&nbsp;<a href="<?=base_url('business-center/post-broiler-trans/' . encode($row->broiler_trans_id).'/'.encode($row->bc_id).'/'.encode($row->broiler_group_id).'/'.encode($row->broiler_group_name))?>" class="brn btn-xs btn-xs glyphicon glyphicon-lock edit-broiler-config" title="Post"></a></td>
										<?php  } else { ?>
										<td class="text-center"><a href="<?=base_url('business-center/view-broiler-trans/' . encode($row->bc_id).'/'. encode($row->broiler_group_id).'/'.encode($row->broiler_group_name) . '/' . encode(date( 'Y', strtotime($row->broiler_trans_date))))?>" class="btn btn-xs btn-success edit-broiler-config">Compute</a></td>
										<?php } ?>
										<!-- <td>&nbsp;&nbsp;<a href="<?=base_url('business-center/remove-broiler-config/' . encode($row->broiler_group_id))?>" class="btn btn-xs glyphicon glyphicon-remove remove-broiler-config" data-id="<?=encode($row->broiler_group_id)?>"></a></td> -->
									</tr>
									<?php endforeach; ?>
					...
					<div id="other-trans-tab" class="tab-pane fade in">
						...
						<div id="add-btn">
							<a href="<?=base_url('business-center/new-broiler-amount-summary/' . encode($bc_id).'/'.$year)?>" class="btn btn-success btn-xs" id="add_broiler_actual_data">+ Add Broiler Actual Data</a>
						</div>
						<div class="table-responsive">
							<table class="table table-hover table-bordered nowrap" id="tbl-other-trans">
								...
								<tbody>
									...
									<tr>
										...
										<td class="text-center"><a href="<?=base_url('business-center/edit-broiler-amount-summary/' . encode($row->broiler_line_item_id).'/'.encode($trans_year_2).'/'.encode($bc_id))?>" class="brn btn-xs glyphicon glyphicon-pencil" title="Edit"></a>&nbsp;&nbsp;<a href="#" data-id="<?=encode($row->broiler_line_item_id)?>" data-bc_id="<?=encode($row->bc_id)?>" data-year="<?=encode($trans_year_2)?>" class="brn btn-xs btn-xs glyphicon glyphicon-remove cancel-broiler-amount-summary" title="Cancel"></a></td>
										<?php  } else { ?>
										<td></td>
										<?php } ?>
									</tr>
					...
					<div id="industry-trans-tab" class="tab-pane fade in">
						...
						<div class="row">
							<div class="col-lg-7">
								...
								<div id="add-btn">
									<a href="<?=base_url('business-center/new-industry-trans/' . encode($bc_id).'/'.encode($year))?>" id="add_industry_trans_button" class="btn btn-success btn-xs">+ Add Industry Data</a>
								</div>
								<div class="table-responsive">
									<table class="table table-hover" id="tbl-industry-trans" width="100%">
										...
										<tbody>
											...
											?>
											<tr>
												<td class="text-center"><a href="#" data-id="<?=encode($row->industry_trans_id)?>" data-bc_id="<?=encode($row->bc_id)?>" data-trans-year="<?=$year?>" class="cancel-industry-trans"><i class="fa fa-remove"></i></a>&nbsp;&nbsp;<a href="<?=base_url('business-center/edit-industry-trans/' . encode($row->industry_trans_id).'/'.encode($bc_id).'/'.encode($year))?>" class=""><i class="fa fa-pencil"></i></a></td>
						    ...
						    <div class="modal-content">
						    	<div class="modal-header">
						      	...
						      	<div class="modal-body">
						      		<form method="POST" action="<?=base_url('business-center/cancel-broiler-amount-summary')?>" enctype="multipart/form-data" id="cancel-broiler-amount-summary">
						...
						<div class="modal-dialog modal-sm">
						    <div class="modal-content">
						    	<div class="modal-header">
						      	...
						      	<div class="modal-body">
						      		<form method="POST" action="<?=base_url('business-center/cancel-industry-trans')?>" enctype="multipart/form-data" id="cancel-industry-trans">
