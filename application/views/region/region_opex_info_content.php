				...
				<div class="row">
					...
					<div class="col-lg-7">
						<div class="text-right">
							<a href="<?=base_url('region/download-opex/' . $cost_center . '/' . $year)?>" target="_blank"><span class="glyphicon glyphicon-download-alt"></span>&nbsp;&nbsp; Download OPEX</a>
						</div>
					</div>
				...
				<ul class="nav nav-tabs">
  				...
  				<div class="tab-content">
    				...
    				<div id="opex-tab" class="tab-pane fade in active"><br/ >
						<div id="add-btn">
							<!-- <a href="<?=base_url('region/transac-opex/' . $id . '/' . $year)?>" class="btn btn-success btn-xs">+ Add Opex</a> -->
						</div>
						<table class="table table-hover" id="tbl-opex-info">
							...
							<tbody>
								...
								<tr>
									...
									<td><?=$row->gl_year?></td>
									<td></td>
									<td><a href="<?=$row->gl_group_name != 'STORE EXPENSES' ? base_url('region/view-opex/' . encode($row->gl_trans_id)) : base_url('region/view-store-expense/' . encode($row->gl_trans_id))?>" class="btn btn-success btn-xs">View</a><!-- &nbsp;&nbsp;<a data-id="<?=encode($row->gl_trans_id)?>" class="btn btn-danger btn-xs cancel-opex-btn">Cancel</a> --></td>
								</tr>
								...
								?>	
									<tr>
										...
										<td></td>
										<td><a href="<?=base_url('region/sw-view/' . encode($row_sw->emp_salary_trans_id))?>" class="btn btn-success btn-xs">View</a><!-- &nbsp;&nbsp;<a data-id="<?=encode($row_sw->emp_salary_trans_id)?>" class="btn btn-danger btn-xs cancel-sw-btn">Cancel</a> --></td>
									</tr>
							    ...
							    <div class="modal-content">
							    	<div class="modal-header">
							      	...
							      	<div class="modal-body">
							      		<form method="POST" action="<?=base_url('region/cancel-opex')?>" enctype="multipart/form-data" id="cancel-opex">
							...
							<div class="modal-dialog modal-sm">
							    <div class="modal-content">
							    	<div class="modal-header">
							      	...
							      	<div class="modal-body">
							      		<form method="POST" action="<?=base_url('region/cancel-sw-opex')?>" enctype="multipart/form-data" id="cancel-sw-opex">
							      			<input type="hidden" name="id" id="id">
								        	<div id="modal-msg" class="text-center">
								        		<label>Are you sure to cancel this OPEX?</label>
								        	</div>
			...
			<script type="text/javascript">
				$(document).ready(function(){
					...
					function amount_per_gl(){
						var url = base_url + 'region/opex-donut/' + id + '/' + year;
						$.ajax({
					    	url: url,
					    	method: 'GET',
					...
					function amount_per_month(){
						var url = base_url + 'region/opex-line/' + id + '/' + year;
						$.ajax({
					    	url: url,
					    	method: 'GET',
					    	success:function(response){
					...
					function amount_per_month_group(){
						var id = $('#id').val();
						var url = base_url + 'region/opex-bar/' + id + '/' + year;
					...
					/*function amount_per_gl_capex(){
						var url = base_url + 'region/opex-donut-capex/' + id;
						$.ajax({
					    	url: url,
					    	method: 'GET',
					    	success:function(response){
					...
					function amount_per_month_capex(){
						var url = base_url + 'region/opex-line-capex/' + id;
						$.ajax({
					    	url: url,
					    	method: 'GET',
					    	success:function(response){
